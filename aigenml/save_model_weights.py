...
def save_model_architecture(model_name, model):
    print(model.get_config())
    with open("{}/{}_config.json".format(model_name, model_name), "w") as f:
        config = clean_dict_helper(model.get_config())
        json.dump(config, f)


def save_model_weights(model_name, model):
    for layer in model.layers:
        ...
        with open("{}/weights/{}.json".format(model_name, layer.name), "w") as f:
